...
def generateAllMoves(board_state):
    ...
    while len(moves_to_do) > 0:
        ...
        for move in getAllMoves(state):
            ...
            new_moves.append([move, new_all_moves, new_state, new_rating])
        # sort new_moves by rating
        new_moves.sort(key=lambda x: x[3], reverse=True)
        # get the top half of the new_moves
        new_moves = new_moves[:int(len(new_moves)/2)]
        moves_to_do.extend(new_moves)
