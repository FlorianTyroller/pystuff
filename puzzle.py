...
empty = 1000
size = 5
field = [[2,3,1],[7,5,4],[empty,8,6]]
#field = [[(i+1)+((j)*size) for i in range(size)]for j in range(size)]
#field[4][2] = empty
0
def isSolved(f):
    last = f[0][0]
...
def getPossibleMoves(f):
    moves = []
    x,y = findField(f)
    for i in range(len(f)):
        for j in range(len(f[0])):
            if f[i][j] != empty and (i == y or j == x):
                moves.append((j,i))
    return moves
                
def makeMove(f,m):
    x,y = findField(f)
    if m in getPossibleMoves(f):
        cx,cy = x,y
...
while not isSolved(field):
    printField(field)
    print(getPossibleMoves(field))
    x = int(input("x: "))
    y = int(input("y: "))
    m = (x,y)
    field = makeMove(field, m)
'''
